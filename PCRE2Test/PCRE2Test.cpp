// PCRE2Test.cpp : This file contains the 'main' function. Program execution begins and ends there.
//
...
vector<FunctionCall> processFile(const string& filepath, pcre2_code* re, pcre2_match_data* match_data) {
    ...
    while ((pos = content.find(marker, pos)) != string::npos) {
        ...
        if (rc >= 0) {
            ...
            // Advance pos to the end of the match in the original content
            pos = start + ovector[1];
        }
        else {
            pos += 1; // No match, move past current character
        }
    }
